const NEGOTIABLE_PRICE_LABEL = "Negotiable";
const PRICE_CATEGORY_ORDER = ["text", "image", "music", "video"];
...
const MODEL_FALLBACK = [
  ...
  },
  {
    ...
    pricing: NEGOTIABLE_PRICE_LABEL,
...
function categoryRank(category) {
  const index = PRICE_CATEGORY_ORDER.indexOf(category);
  return index === -1 ? PRICE_CATEGORY_ORDER.length : index;
}

function docsUrlForCategory(category) {
  if (category === "image") return "api.html#image-generation";
  return category === "text" ? "api.html#text-generation" : "api.html#model-list";
}

function inferCategory(modelName) {
...
function categoryFor(modelName, categories) {
  if (categories.music?.includes(modelName)) return "music";
  if (categories.video?.includes(modelName)) return "video";
  if (categories.image?.includes(modelName)) return "image";
  if (categories.text?.includes(modelName)) return "text";
  return inferCategory(modelName) || categories.defaultCategory || "text";
}
...
function normalizeModelInfo(upstream) {
  ...
  const models = rows.map((row) => {
    ...
    return {
      ...
      pricing: category === "text" && !isNegotiablePrice(input) && !isNegotiablePrice(output)
      ...
      docsUrl: docsUrlForCategory(category),
...
function sortPrices(prices) {
  ...
  if (state.priceSort === "price") {
    return list.sort((a, b) => {
      const aPrice = a.category === "text" && !isNegotiablePrice(a.inputPerMillionTokens) ? a.inputPerMillionTokens : undefined;
      const bPrice = b.category === "text" && !isNegotiablePrice(b.inputPerMillionTokens) ? b.inputPerMillionTokens : undefined;
...
function summaryPriceLabel(value) {
  return isNegotiablePrice(value) ? label(NEGOTIABLE_PRICE_LABEL) : `${label("From")} ${usd(value)}`;
}

function summaryUnitLabel(value) {
  return isNegotiablePrice(value) ? label("Commercial terms pending") : label("per 1M tokens");
}

function minDefined(values) {
  const defined = values.filter((value) => !isNegotiablePrice(value));
...
function renderPricingSummary(prices) {
  ...
  summary.innerHTML = `
    <article class="card price-card">
      <span class="badge">${label("Text input")}</span>
      <strong>${summaryPriceLabel(lowestInput)}</strong>
      <p>${summaryUnitLabel(lowestInput)}</p>
    </article>
    <article class="card price-card">
      <span class="badge">${label("Text output")}</span>
      <strong>${summaryPriceLabel(lowestOutput)}</strong>
      <p>${summaryUnitLabel(lowestOutput)}</p>
    </article>
  `;
}
