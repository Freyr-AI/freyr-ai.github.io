...
function publicFileUrl(path) {
  const normalized = String(path).replace(/^\/freyr\/files\//, "/files/");
  return absoluteUrl(`/api/native${normalized}`);
}
...
function mediaUrlCandidates(value) {
  ...
  if (/^https?:\/\//i.test(text)) {
    try {
      ...
      if (url.pathname.startsWith("/api/native/freyr/files/")) {
        return [absoluteUrl(`${url.pathname.replace("/api/native/freyr/files/", "/api/native/files/")}${url.search}${url.hash}`)];
      }
      if (url.pathname.startsWith("/files/") || url.pathname.startsWith("/freyr/files/")) {
  ...
  if (text.startsWith("/api/native/freyr/files/")) {
    return [absoluteUrl(text.replace("/api/native/freyr/files/", "/api/native/files/"))];
  }
  if (text.startsWith("/api/")) return [absoluteUrl(text)];
  if (text.startsWith("/files/") || text.startsWith("/freyr/files/")) return [publicFileUrl(text)];
