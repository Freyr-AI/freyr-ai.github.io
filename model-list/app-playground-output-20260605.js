...
const MEDIA_POLL_TIMEOUT_MS = 10 * 60 * 1000;
...
function defaultPrompt(category) {
  if (category === "image") return "Create a cinematic product hero image of a matte black electric bicycle parked on a rain-wet Tokyo street at blue hour, with neon reflections, realistic lighting, shallow depth of field, and no text.";
...
async function waitForMediaEntry(entry, index, total) {
  ...
  while (Date.now() - startedAt < MEDIA_POLL_TIMEOUT_MS) {
    ...
    setResult(generationMessage(`Polling output URL for ${label}. Attempt ${attempt}.`));
    try {
...
async function waitForMediaOutputs(json, category) {
  ...
  for (let index = 0; index < entries.length; index += 1) {
    await waitForMediaEntry(entries[index], index, entries.length);
  }
}
...
function bindEvents() {
  $("#apiOriginLabel").textContent = DEFAULT_ORIGIN;
