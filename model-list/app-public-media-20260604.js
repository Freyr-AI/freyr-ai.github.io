...
function publicFreyrFileUrl(path) {
...
function mediaUrlCandidates(value) {
  ...
  if (/^https?:\/\//i.test(text)) {
    try {
      ...
      if (url.pathname.startsWith("/freyr/files/")) {
        return [publicFreyrFileUrl(`${url.pathname}${url.search}${url.hash}`)];
      }
    } catch {
      return [text];
    }
  ...
  if (text.startsWith("/freyr/files/")) return [publicFreyrFileUrl(text)];
...
function mediaPreviewMarkup(entry) {
  ...
  if (/^data:/i.test(entry.url) && directPreview) return directPreview;

  return `
    <div class="media-frame" data-media-url="${safeUrl}" data-media-type="${escapeHtml(entry.type)}">
      <span class="media-status">Loading ${escapeHtml(previewLabel(entry.type))} preview...</span>
    </div>
  `;
}
...
async function hydrateMediaFrame(frame, signal) {
  const url = frame.dataset.mediaUrl;
  if (!url) return;
...
function hydrateMediaPreviews(root) {
  const frames = $$("[data-media-url]", root);
...
function renderMedia(json, category) {
  ...
  if (outputEntries.length) {
    return `<div class="media-output">${outputEntries.map((entry, index) => mediaItemMarkup(entry, index, entry.url)).join("")}</div>`;
  }
...
async function downloadMedia(url, filename, button) {
  ...
  try {
    ...
    button.title = `Fetch download failed: ${friendlyError(error)}`;
    window.setTimeout(() => {
