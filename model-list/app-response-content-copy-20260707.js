...
function resultTextForCopy() {
  ...
  return (result.querySelector(".response-copy-source") || result).innerText.trim();
}
  ...
  while (true) {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        setResult(`<div class="response-copy-source"><strong>Streaming response</strong><pre class="response-copy-content">${escapeHtml(output)}</pre></div>${usageHtml(json)}`);
      } catch {
        output += data;
      }
    }
